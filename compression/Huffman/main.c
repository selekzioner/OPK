...
int main(int argc, char* argv[]) {
	if (argc != _ARG_COUNT && argc != 1 /*|| (strcmp(argv[1], "-c") != 0 && strcmp(argv[1], "-u") != 0)*/) {
		...
		if (strcmp(argv[1], "-c") == 0) {
			Huffman_encoding(argv[2], argv[3]);
		}
		else if (strcmp(argv[1], "-u") == 0) {
			Huffman_decoding(argv[2], argv[3]);
		}
	}
	/*clock_t start = clock();
	Huffman_encoding("Voina_i_mir.txt", "Voina_i_mir.bin");
	clock_t end = clock();
	double time = (double)(end - start) / CLOCKS_PER_SEC;
	printf("time: %.4lf sec\n", time);

	start = clock();
	Huffman_decoding("Voina_i_mir.bin", "Voina_i_mir_out.txt");
	end = clock();
